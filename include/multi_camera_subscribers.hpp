...
public:
	struct Parameters {
		int queue_size;
		std::array<std::string, N> data_topics;
	};
...
private:
	...
	// subscriber
	std::array<image_transport::SubscriberFilter, N> image_subs_;
	std::array<message_filters::Subscriber<sensor_msgs::CameraInfo>, N> info_subs_;
	

	// for sync checking
	ros::WallTimer check_synced_timer_;
	std::array<int, N> data_received_;
	int all_received_;

	using ExactPolicy = message_filters::sync_policies::ExactTime<>; //<sensor_msgs::Image... T2, typename std::enable_if<sizeof...(T2) == 2, int>::type = 0 >;

	message_filters::Synchronizer<ExactPolicy> exact_sync_;

	// using ExactPolicy = message_filters::sync_policies::ExactTime<sensor_msgs::Image... >;
...
protected:
	...
	MultiCameraProcessor(const Parameters& param) :  param_(param), exact_sync_(ExactPolicy(param.queue_size))
	{
